...
jq '.jmxMetrics = ([.jmxMetrics[] | (range(0;3)|tostring) as $d | (.objectName|startswith("metrics:name=databricks.0.")) as $f | .objectName=if $f then (.objectName | "metrics:name=databricks." + $d + "." + ltrimstr("metrics:name=databricks.0.")) else .objectName end]|unique)' /tmp/template.json > /tmp/applicationinsights.json
